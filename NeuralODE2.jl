### Author: Marcel Romaní, based on Long Nguyen's

### Slight change to NeuralODE: now the initial condition is a parameter of the NODE
### which is also going to be tuned in the optimization.
...
function Zygote._zero(xs::Vector{<:Number}, T=Any)
    return [Zygote._zero(x) for x in xs]
end

function Zygote._zero(xs::Matrix{<:Number}, T=Any)
    return [Zygote._zero(x) for x in xs]
end

function Zygote._zero(x::Float32, T=Any)
    return 0.0f0
end

function Zygote._zero(x::Float64, T=Any)
    return 0.0e0
end
...
struct NeuralODE{F, Z, P, T, S}
    f::F     # derivative model
    z_t0::Z    # initial condition
...
function NeuralODE(f, z_t0, θ, tspan)
    return NeuralODE(f, z_t0, θ, tspan, DiffEqBase.AbstractODESolution[])
end
...
# We also specify the parameters `θ` and the initial condition 'z_t0' to be 
# optimized with `Flux.trainable`.
Flux.trainable(node::NeuralODE) = (node.z_t0, node.θ, )
...
function (node::NeuralODE)(; alg=Tsit5(), kwargs...)
    f, z_t0, θ, t0, t1, sol = node.f, node.z_t0, node.θ, node.tspan[1], node.tspan[2], node.sol
...
function _backward(∂L∂z_t1, θ, t0, t1; f, sol, alg)
    ...
    function dsdt(ds, s, θ, t)
        ...
        get_derivative(Δ, x) = (isnothing(Δ) ? zero(x) : Δ)
